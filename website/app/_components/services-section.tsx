  ...
  return (
    <section id="services" className="relative py-28 overflow-hidden bg-[var(--bg)]">
      <style>{`
        ...
        /* Иконки — темно-синий var(--primary) */
        .service-icon {
        ...
        /* Номера-водяные знаки — темно-синие */
        .watermark {
          color: var(--primary);
          opacity: 0.12;
        }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((s, i) => {
            ...
            return (
              ...
              >
                {/* Номер — водяной знак */}
                <span className="watermark absolute top-4 right-6 text-[72px] lg:text-[88px] font-heading font-black leading-none select-none pointer-events-none">
                  {s.num}
                </span>

                {/* Цветная полоска слева при hover — темно-синяя */}
                <div className="service-accent-line absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-0 group-hover:h-16 rounded-r-full transition-all duration-500 ease-out" />
                ...
                <div className="mt-8 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                  <span className="text-caption font-medium text-[var(--primary)]">
                    Подробнее
                  </span>
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none" className="text-[var(--primary)]">
